...
//
// WebServer Copyright 2015-2016 Espressif Systems (Shanghai) PTE LTD
//
...
/*<! 1: detect by two-stage which is more accurate but slower(with keypoints). */
#define TWO_STAGE 1                     
...
static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results)
{
    ...
    if(recognize.id >= 0){
        ...
        rgb_print(fb, FACE_COLOR_RED, "Warning: Opponent");
    }
    return recognize.id;
}
#endif
...
esp_err_t toggle_led_handler(httpd_req_t *req) {
    ...
    //led_on = !led_on;
    //ledcSetup(1, 5000, 8); // Configure PWM
    //ledcAttachPin(21, 1);  // Attach the onboard LED pin
    //ledcWrite(1, 255);     // Turn LED fully ON (100% duty)
    
    static bool led_on = false;
    
    const gpio_num_t led_gpio = GPIO_NUM_21; // Adjust if your onboard LED is on a different GPIO
    gpio_pad_select_gpio(led_gpio);
    gpio_set_direction(led_gpio, GPIO_MODE_OUTPUT);
    led_on = !led_on;
    gpio_set_level(led_gpio, led_on ? 1 : 0);
    
    httpd_resp_send(req, led_on ? "LED ON" : "LED OFF", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

esp_err_t forward_handler(httpd_req_t *req) {    
 
    const gpio_num_t pin0 = GPIO_NUM_1;
    const gpio_num_t pin1 = GPIO_NUM_2;
    const gpio_num_t pin2 = GPIO_NUM_3;
    const gpio_num_t pin3 = GPIO_NUM_4; 
...
esp_err_t backward_handler(httpd_req_t *req) {    
    ...
    httpd_resp_send(req, "Backward", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
void startCameraServer()
{
...
#endif
    log_i("RoboSoccer WebServer in on port: '%d'", config.server_port);
    ...
    config.ctrl_port += 1;
    log_i("RoboSoccer LiveStream is on port: '%d'", config.server_port);
...
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.Espressif Systems (Shanghai) PTE LTD
